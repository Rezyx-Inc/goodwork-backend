...
class RecruiterController extends Controller
{
        ...
        if ($validator->fails()) {
            ...
            if ($user_info->count() > 0) {
                ...
                foreach ($job_data as $draft) {
                    ...
                    $result['organization_weekly_amount'] = isset($draft['organization_weekly_amount']) ? $draft['organization_weekly_amount'] : "";
        ...
        if ($validator->fails()) {
            ...
            if ($user_info->count() > 0) {
                ...
                foreach ($job_data as $published) {
                    ...
                    $result['organization_weekly_amount'] = isset($published['organization_weekly_amount']) ? $published['organization_weekly_amount'] : "";
        ...
        if ($validator->fails()) {
            ...
            if ($user_info->count() > 0) {
                ...
                if ($worker_info->count() > 0) {
                    ...
                    $respond = Nurse::select('keywords.filter as job_filter', 'keywords.title as job_title', 'nurses.*', 'jobs.*', 'offers.job_id as job_id', 'offers.id as offer_id', 'user_id as worker_user_id', 'offers.nurse_id as worker_id', 'users.first_name', 'users.last_name', 'users.image', 'facilities.name as organization_name', 'offers.status as offer_status', 'offers.start_date as status_date', 'offers.expiration as status_enddate')
            ...
            if ($user_info->count() > 0) {
                ...
                if ($request->status == 'Offered') {
                    if (isset($records)) {
                        ...
                        $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved from Submitted to Offered by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                    ...
                    if (isset($records)) {
                        ...
                        if ($check_rec['position_available'] > $is_vacancy) {
                            ...
                            $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved from Offered to Onboarding by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                            $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                            if ($status != 0) {
                                if (empty($check)) {
                                    $notification = $fields->save();
                    ...
                    if (isset($records)) {
                        ...
                        $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved from Working to Done by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                        $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                    ...
                    if (isset($records)) {
                        ...
                        $fields->title = 'Done';
                        $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was Done by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                        $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                        if ($status != 0) {
                            if (empty($check)) {
                                $notification = $fields->save();
                    ...
                    if (isset($records)) {
                        ...
                        $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved from Screening to Submitted by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                        $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                        if ($status != 0) {
                            if (empty($check)) {
                                $notification = $fields->save();
                    ...
                    if (isset($records)) {
                        ...
                        $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved to Rejected by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                        $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                        if ($status != 0) {
                            if (empty($check)) {
                                $notification = $fields->save();
                    ...
                    if (isset($records)) {
                        if (isset($request->recruiter_id)) {
                            ...
                            $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved to Blocked by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                            $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                            if ($status != 0) {
                                if (empty($check)) {
                                    $notification = $fields->save();
                    ...
                    if (isset($records)) {
                        ...
                        $fields->text = 'Your Application for job ' . $records['job_id'] . ' at Medical Solutions Recruiter was moved from New to Screening by ' . $user->first_name . ' ' . $user->last_name . ' from Organization name';
                        $check = Notification::where(['job_id' => $records['job_id'], 'created_by' => $records['nurse_id']])->first();
                        if ($status != 0) {
                            if (empty($check)) {
                                $notification = $fields->save();
                ...
                if ($worker_info->count() > 0) {
                    ...
                    if (isset($offer_check)) {
                        if ($user_info->count() > 0) {
                            ...
                            if ($job_data['nursing_license_state'] == $job_data['job_location']) {
                                $val = true;
                            } else {
                                $val = false;
                            }
                            ...
                            $data['worker'] = isset($job_data['worker_weeks_assignment']) ? $job_data['worker_weeks_assignment'] : "-";
                            $data['name'] = 'Assignment in weeks';
                            $data['worker1'] = isset($job_data['worker_organization_weekly_amount']) ? $job_data['worker_organization_weekly_amount'] : "-";
        ...
        if ($validator->fails()) {
            ...
            if ($user_info->count() > 0) {
                ...
                foreach ($job_data as $rec) {
                    ...
                    $result['worker_name'] = isset($rec['first_name']) ? $rec['first_name'] . ' ' . $rec['last_name'] : "";
                    $result['facility_name'] = isset($rec['facility_name']) ? $rec['facility_name'] : "";
                    $result['organization_weekly_amount'] = isset($rec['organization_weekly_amount']) ? $rec['organization_weekly_amount'] : "";
    ...
    public function applicationInfo(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $data['job'] = isset($job_data['organization_average_rating']) ? $job_data['organization_average_rating'] : "-";
            $data['name'] = "Organization Average Rating";
            ...
            $data['name'] = "Weekly non-taxable amount";
            $data['job1'] = isset($job_data['organization_weekly_amount']) ? $job_data['organization_weekly_amount'] : "-";
            $data['name1'] = 'Organization Weekly Amount';
            $worker_info[] = $data;

            $data['job'] = isset($job_data['goodwork_weekly_amount']) ? $job_data['goodwork_weekly_amount'] : "-";
            $data['name'] = "Goodwork Weekly Amount";
            $data['job1'] = isset($job_data['total_organization_amount']) ? $job_data['total_organization_amount'] : "-";
            $data['name1'] = 'Total Organization Amount';
            $worker_info[] = $data;
    ...
    public function getJobKeys(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (isset($jobs)) {
                ...
                $job_data["organization_average_rating"] = isset($jobs['organization_average_rating']) ? $jobs['organization_average_rating'] : '';
                ...
                $job_data["organization_weekly_amount"] = $job_data["weekly_taxable_amount"] + $job_data["weekly_non_taxable_amount"];
                $job_data["goodwork_weekly_amount"] = $job_data["organization_weekly_amount"] * 0.05;
                $job_data["total_organization_amount"] = ($job_data["preferred_assignment_duration"] * $job_data["organization_weekly_amount"]) + $job_data["sign_on_bonus"] + $job_data["completion_bonus"];
                $job_data["total_goodwork_amount"] = $job_data["preferred_assignment_duration"] * $job_data["goodwork_weekly_amount"];
                $job_data["total_contract_amount"] = $job_data["total_organization_amount"] + $job_data["total_goodwork_amount"];
                $job_data["additional_terms"] = isset($jobs['additional_terms']) ? $jobs['additional_terms'] : '';
                $job_data['weekly_pay'] = $job_data["organization_weekly_amount"];
                if ($job_data["hours_per_week"] == 0) {
                    $job_data["feels_like_per_hour"] = 0;
                } else {
                    $job_data["feels_like_per_hour"] = $job_data["organization_weekly_amount"] / $job_data["hours_per_week"];
                }
    ...
    { {
            ...
            if ($validator->fails()) {
                ...
                if (isset($job)) {
                    ...
                    $popup['name'] = '';
                    $popup['value'] = '';
                    if (($job->organization_average_rating != $job_data['organization_average_rating']) && isset($job->organization_average_rating) && !empty($job->organization_average_rating)) {
                        $popup['name'] = "Organization Average Rating";
                        $popup['value'] = $job->organization_average_rating;
                        $jobs["name"] = "Organization Average Rating";
                        $jobs["job"] = isset($job->organization_average_rating) ? $job->organization_average_rating : '';
                    } else {
                        $jobs["name"] = "Organization Average Rating";
                        $jobs["job"] = isset($job_data['organization_average_rating']) ? $job_data['organization_average_rating'] : '';
                    }
                    $popup_info[] = $popup;
                    $popup['name'] = '';
                    $popup['value'] = '';
                    ...
                    $popup['name'] = '';
                    $popup['value'] = '';
                    if (($job->organization_weekly_amount != $job_data['organization_weekly_amount']) && isset($job->organization_weekly_amount) && !empty($job->organization_weekly_amount)) {
                        $popup['name'] = "Organization Weekly Amount";
                        $popup['value'] = $job->organization_weekly_amount;
                        $jobs["name1"] = "Organization Weekly Amount";
                        $jobs["job1"] = $job->organization_weekly_amount;
                    } else {
                        $jobs["name1"] = "Organization Weekly Amount";
                        $jobs["job1"] = $job_data['organization_weekly_amount'];
                    }
                    $worker_info[] = $jobs;
                    $popup_info[] = $popup;
                    $popup['name'] = '';
                    ...
                    $popup['name'] = '';
                    $popup['value'] = '';
                    if (($job->total_organization_amount != $job_data['total_organization_amount']) && isset($job->total_organization_amount) && !empty($job->total_organization_amount)) {
                        $popup['name'] = "Total Organization Amount";
                        $popup['value'] = $job->total_organization_amount;
                        $jobs["name1"] = "Total Organization Amount";
                        $jobs["job1"] = $job->total_organization_amount;
                    } else {
                        $jobs["name1"] = "Total Organization Amount";
                        $jobs["job1"] = $job_data['total_organization_amount'];
                    }
                    $worker_info[] = $jobs;
                    $popup_info[] = $popup;
                    $popup['name'] = '';
        ...
        if ($validator->fails()) {
            ...
            if (isset($offer)) {
                ...
                if (isset($job)) {
                    ...
                    if (($job->organization_average_rating != $job_data['organization_average_rating']) && isset($job->organization_average_rating) && !empty($job->organization_average_rating)) {
                        $popup['name'] = "Organization Average Rating";
                        $popup['value'] = isset($job->organization_average_rating) ? $job->organization_average_rating : '';
                        $jobs["name"] = "Organization Average Rating";
                        $jobs["job"] = isset($job->organization_average_rating) ? $job->organization_average_rating : '';
                    } else {
                        $jobs["name"] = "Organization Average Rating";
