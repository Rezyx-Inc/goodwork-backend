    ...
    <form action="{{ route('my-profile.store') }}" class="redirect-after-submit">
            ...
            <select name="facility_shift_cancelation_policy">
                ...
                @foreach ($assignment_durations as $v)
                    <option value="{{ $v->title }}"
                        {{ $model->facility_shift_cancelation_policy == $v->title ? 'selected' : '' }}>{{ $v->title }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="ss-form-group">
            ...
            <select name="contract_termination_policy">
                ...
                @foreach ($contract_policies as $v)
                    <option value="{{ $v->title }}"
                        {{ $model->contract_termination_policy == $v->title ? 'selected' : '' }}>{{ $v->title }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="ss-form-live-location">
            <div class="ss-form-group">
                ...
                <input type="text" value="{{ $model->distance_from_your_home }}" name="distance_from_your_home"
        ...
        <div class="ss-form-group">
            ...
            <select name="facilities_you_like_to_work_at" class="form-control">
                ...
                @foreach ($facilities as $v)
                    <option value="{{ $v->id }}"
                        {{ $model->facilities_you_like_to_work_at == $v->id ? 'selected' : '' }}>{{ $v->name }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="ss-form-group">
            ...
            <input type="text" value="{{ $model->worker_facility_parent_system }}" name="worker_facility_parent_system"
        ...
        <div class="ss-form-group">
            ...
            <input type="text" value="{{ $model->avg_rating_by_facilities }}" class="form-control"
                name="avg_rating_by_facilities" placeholder="Your average rating by your facilities">
        </div>

        <div class="ss-form-group">
            <label>Recruiter Average Rating</label>
            <input type="text" value="{{ $model->worker_avg_rating_by_recruiters }}" class="form-control"
                name="worker_avg_rating_by_recruiters" placeholder="Your average rating by your recruiters">
        </div>

        <div class="ss-form-group">
            <label>Organization Average Rating</label>
            <input type="text" value="{{ $model->worker_avg_rating_by_organizations }}" class="form-control"
                name="worker_avg_rating_by_organizations" placeholder="Your average rating by your organizations">
        </div>
        ...
        <div class="ss-form-group">
            ...
            <select name="clinical_setting_you_prefer" class="form-control">
                ...
                @foreach ($types as $v)
                    <option value="{{ $v->title }}"
                        {{ $model->clinical_setting_you_prefer == $v->title ? 'selected' : '' }}>{{ $v->title }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="ss-prsn-form-btn-sec">
            <button type="button" class="ss-prsnl-skip-btn" id="skip-button" data-href="{{ route('patient-ratio') }}"
