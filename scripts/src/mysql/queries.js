...
module.exports.getLaboredgeLogin = async function (userId){
	const [result, fields] = await pool.query("SELECT * from laboredge WHERE user_id=?;", [userId]);
	return result
}

module.exports.closeImportedJobs = async function (imported_id){
	const [result, fields] = await pool.query("UPDATE jobs SET is_open=0, is_closed=1 WHERE import_id=?;", [imported_id]);
	return result
}

module.exports.addImportedJob = async function (importData){

	console.log("IMPORT IN QUERY")
	...
	let description = importData.postingId + " " + importData.description + " " + importData.shift;
