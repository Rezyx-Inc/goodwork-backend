        ...
        <ul class="ssnavbar-nav">
          ...
          <li class="nav-item dropdown ss-bell-sec-li d-none">
            <a class="ss-bell-sec nav-link me-3 me-lg-0 dropdown-toggle hidden-arrow" href="#" id="navbarDropdownMenuLink" role="button" data-mdb-toggle="dropdown" aria-expanded="false">
              ...
              {{-- <span id="notifications_counter_recruiter" class="badge rounded-pill badge-notification bg-danger header_notif">{{$unreadNotificationsCount}}</span> --}}
            ...
            <ul id="list_of_notifications" class="dropdown-menu dropdown-menu-end " aria-labelledby="navbarDropdownMenuLink">
              ...
              {{-- <li><a id="offer_notification" class="d-none dropdown-item" href="{{route('recruiter-opportunities-manager')}}">New Jobs Information</a></li>
              <li>
                <a id="offer_notification" class="d-none dropdown-item" href="{{route('recruiter-application')}}">You got new offer update !</a>
              </li> --}}
            </ul>
          </li>
...
function handleNotificationClick(event) {
    ...
    $.ajax({
        url: '{{ route("read-recruiter-message-notification") }}', // Replace 'read-message-notification' with the actual path to your route
        type: 'POST',
        data: data,
        success: function(response) {
            console.log('Success:', response);
            ...
            window.location.href = '{{ route("recruiter-messages") }}';
        },
        error: function(xhr, status, error) {
            console.error('Error:', error);
            // Handle errors here
...
function handleJobNotificationClick(event , senderId){
  ...
  // Send the ID to your reading route using jQuery's $.ajax
  $.ajax({
      url: '{{ route("read-recruiter-job-notification") }}', // Replace 'read-message-notification' with the actual path to your route
      type: 'POST',
      data: data,
      success: function(response) {
          console.log('Success:', response);
