    ...
    <div class="row d-flex justify-content-center align-items-center">
        <div class="col-9">
            ...
            <select name="status" id="status application-status">
                ...
                <option value="Screening" {{ $offerdetails['status'] === 'Screening' ? 'selected hidden disabled' : '' }}>Screening
                </option>
            ...
            <select name="status" id="status application-status">
                ...
                <option value="Submitted" {{ $offerdetails['status'] === 'Submitted' ? 'selected hidden disabled' : '' }}>Submitted
                </option>
                <option value="Offered" {{ $offerdetails['status'] === 'Offered' ? 'selected hidden disabled' : '' }}>Offered</option>
                <option value="Done" {{ $offerdetails['status'] === 'Done' ? 'selected hidden disabled' : '' }}>Done</option>
                <option value="Onboarding" {{ $offerdetails['status'] === 'Onboarding hidden disabled' ? 'selected' : '' }}>
                    Onboarding</option>
                {{-- <option value="Working" {{ $offerdetails['status'] === 'Working' ? 'selected' : '' }}>Working
                </option> --}}
                <option value="Rejected" {{ $offerdetails['status'] === 'Rejected' ? 'selected hidden disabled' : '' }}>Rejected
                </option>
                <option value="Blocked" {{ $offerdetails['status'] === 'Blocked' ? 'selected hidden disabled' : '' }}>Blocked
                </option>
                <option value="Hold" {{ $offerdetails['status'] === 'Hold' ? 'selected hidden disabled' : '' }}>Hold</option>
            </select>
            @endif
        </div>
        <div class="col-3">
...
<div class="ss-jb-apl-oninfrm-mn-dv">
    <ul class="ss-jb-apply-on-inf-hed-rec row">
        ...
        <div class="row {{ $offerdetails->employer_weekly_amount === $userdetails->nurse->worker_employer_weekly_amount ? 'ss-s-jb-apl-bg-blue' : 'ss-s-jb-apl-bg-pink' }} d-flex align-items-center"
            style="margin:auto;">
            <div class="col-md-6">
                <h6>{{ number_format($offerdetails->employer_weekly_amount) ?? '----' }}</h6>
            </div>
            <div class="col-md-6 {{ $offerdetails->employer_weekly_amount ? '' : 'd-none' }}">
                <p>
                    {!! number_format($userdetails->nurse->worker_employer_weekly_amount) ??
                        '<a style="cursor: pointer;" onclick="askWorker(this, \'worker_employer_weekly_amount\', \'' .
                            $userdetails->nurse['id'] .
                            '\', \'' .
                            $offerdetails['id'] .
                            '\')">Ask Worker</a>' !!}
        ...
        <div class="col-md-12">
            <h6>{{ number_format($offerdetails->total_employer_amount) ?? '----' }}</h6>
        </div>
        <div class="col-md-12">
