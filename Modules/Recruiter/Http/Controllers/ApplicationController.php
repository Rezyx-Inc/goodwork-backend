...
public function getApplicationListing(Request $request)
                ...
                $data .= '
                    <div class="ss-job-prfle-sec ' . ( $request->id == $value->id ? 'active' : '') . '" id="ss-expl-applicion-div-box" onclick="applicationType(\'' . $type . '\', \'' . $value->id . '\', \'userdetails\')">
                        <div class="ss-job-id-no-name">
                            <ul>
                                <li class="w-50">
                                    <span href="" class="mb-3">' . $value->worker_user_id . '</span>
                                </li>
                                <li class="w-50">
                                    <p>Recently Added</p>
                                </li>
                        ...
                        <ul>
                            <li>                                                                                                                                
                                <img src="' . asset('public/images/nurses/profile/' . $user['image']) . '" onerror="this.onerror=null;this.src=' . '\'' . URL::asset('frontend/img/profile-pic-big.png') . '\'' . ';" id="preview" width="50px" height="50px" style="object-fit: cover;" class="rounded-3" alt="Profile Picture">
                            </li>
                            <li>
                                <h6></h6>
                            </li>
            ...
            if ($request->formtype == 'useralldetails') {
                        ...
                        <li>';
                            ...
                            if($request->type == 'Apply'|| $request->type == 'Screening' || $request->type == 'Submitted'){
                                ...
                                $data2 .= '
                                    ...
                                    <p>data to sent : type :'. $type .' ,offer id :' . $request->id .' ,form type : '.$request->formtype.' ,jobId: '.$request->jobid.'</p>
                                ';
                            }
                            $data2 .= '
                        </li>
                ...
                $data2 .= '
                    ...
                    <div class="ss-form-group">
                        ...
                        <input type="text" name="contract_termination_policy" placeholder="Enter Contract Termination Policy" value="' . (isset($jobdetails['contract_termination_policy']) ? $jobdetails['contract_termination_policy'] : '2 weeks of guaranteed pay unless canceled for cause') . '">
                    </div>
                    <div class="ss-form-group">
                    ...
                    <div class="ss-form-group">
                        ...
                        <input type="text" name="clinical_setting" placeholder="Enter clinical setting" value="' . $jobdetails['clinical_setting'] . '">
                    </div>
                    <div class="ss-form-group">
                    ...
                    $offerdetails = DB::table('offers')->where(['job_id' => $request->jobid, 'worker_user_id' => $userdetails->id])->first();
                    $data2 .= '
                        <div class="ss-jb-apl-oninfrm-mn-dv">
                            <div>
                                <h4><img src="'.asset('public/recruiter/assets/images/counter-left-img.png').'"> Send Offer</h4>
    ...
    <ul class="ss-cng-appli-hedpfl-ul">
        <li>
            <span>' . $offerdetails['worker_user_id'] . '</span>
            <h6>
                <img src="' . asset('public/images/nurses/profile/' . $userdetails->image) . '" onerror="this.onerror=null;this.src=' . '\'' . URL::asset('frontend/img/profile-pic-big.png') . '\'' . ';" id="preview" width="50px" height="50px" style="object-fit: cover;" class="rounded-3" alt="Profile Picture">
                ' . $userdetails->first_name . ' ' . $userdetails->last_name . '
            </h6>
        </li>
        <li>
            <a href="' . route('recruiter-messages') . '" class="rounded-pill ss-apply-btn py-2 border-0 px-4" onclick="chatNow(\'' . $offerdetails['worker_user_id'] . '\')">Chat Now</a>
            <p>data to sent : type :'. $type .' ,offer id :' . $request->id .' ,form type : '.$request->formtype.' ,jobId: '.$request->jobid.'</p>
        </li>
    </ul>
    ...
    foreach ($jobapplieddetails as $key => $value) {
        if ($value) {
            ...
            $data2 .= '
                <div style="width:100%;" class="ss-chng-appli-slider-sml-dv" onclick="applicationType(\'' . $type . '\', \'' . $value->id . '\', \'useralldetails\', \''. $value->job_id .'\')">
...
public function updateApplicationStatus(Request $request){
    ...
    public function sendJobOffer(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // $nurse = Nurse::where('id', $request->worker_user_id)->first();
            ...
            $offerexist = DB::table('offers')->where(['job_id' => $request->job_id, 'worker_user_id' => $request->worker_user_id, 'recruiter_id' => $request->recruiter_id])->first();
            if($offerexist){
                $job = DB::table('offers')->where(['job_id' => $request->job_id, 'worker_user_id' => $request->worker_user_id, 'recruiter_id' => $request->recruiter_id])->update($update_array);
            }else{
    ...
    public function sendJobOfferRecruiter(Request $request){
        ...
        if ($validator->fails()) {
            ...
            $job = DB::table('offers')->where(['id' => $request->id])->first();
            if($job->is_draft == "0"){
                $responseData = [
                    'status' => 'success',
                    'message' => 'Job Offer Already Send',
                ...
                $job = DB::table('offers')->where(['id' => $request->id])->update($update_array);
                if ($job) {
                    $responseData = [
                        'status' => 'success',
