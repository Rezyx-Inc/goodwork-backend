...
class OrganizationOpportunitiesController extends Controller
{
    ...
    // Used to feed jobs to the infinite scroll
    public function load_more_jobs(Request $request)
    {
        $organization_id = Auth::guard('organization')->user()->id;
        $scriptResponse = Http::get('http://localhost:'. config('app.file_api_port') .'/organizations/getRecruiters/' . $organization_id);
        $api_response = json_decode($scriptResponse->body());
        ...
        try {
            ...
            return response()->json(['success' => true, 'message' =>  $resp]);

        } catch (\Exception $e) {
            return response()->json(['error' => 'Unable to load more jobs, please try again later.']);
        }
    }
