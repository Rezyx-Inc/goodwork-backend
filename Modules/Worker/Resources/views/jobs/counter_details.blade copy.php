...
<div class="ss-job-apply-on-view-detls-mn-dv">
    ...
    <ul class="ss-jb-apply-on-inf-hed-rec row">
        ...
        <li
            class="col-md-6 mb-3 {{ $jobdetails->profession != $offerdetails->profession ? 'ss-job-view-off-text-fst-dv' : '' }}">
        ...
        @if (isset($jobdetails->specialty))
            @foreach (explode(',', $offerdetails->specialty) as $value)
                @if ($value)
                    <div
                        class="col-md-6 mb-3 {{ $jobdetails->specialty != $value ? 'ss-job-view-off-text-fst-dv' : '' }}">
        ...
        @endif
        @dump($jobdetails)
        @foreach (['block_scheduling', 'float_requirements', 'facility_shift_cancellation_policy', 'contract_termination_policy', 'traveler_distance_from_facility', 'clinical_setting', 'patient_ratio', 'emr', 'unit', 'scrub_color', 'interview_dates', 'start_date', 'rto', 'hours_per_week', 'guaranteed_hours', 'hours_shift', 'weeks_assignment', 'shifts_week', 'referral_bonus', 'sign_on_bonus', 'completion_bonus', 'extension_bonus', 'other_bonus', 'four_zero_one_k', 'health_insurance', 'dental', 'vision', 'actual_hourly_rate', 'overtime', 'holiday', 'on_call', 'orientation_rate', 'est_weekly_taxable_amount', 'est_organization_weekly_amount', 'est_weekly_non_taxable_amount', 'est_goodwork_weekly_amount', 'est_total_organization_amount', 'est_total_goodwork_amount', 'est_total_contract_amount'] as $item)
            <div
                class="col-md-6 mb-3 {{ $jobdetails->$item != $offerdetails->$item ? 'ss-job-view-off-text-fst-dv' : '' }}">
    ...
    <div class="ss-job-apl-on-offer-btn">
        <button class="ss-acpect-offer-btn" data-offer_id="{{ $offer_id }}" onclick="accept_job_offer(this)">Accept
            Offer</button>
        <ul>
