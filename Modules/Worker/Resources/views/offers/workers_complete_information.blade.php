<ul class="ss-cng-appli-hedpfl-ul">
    ...
    @else --}}
        <li>
            <a onclick="askRecruiter(this, 'nursing_profession', '{{ $nursedetails['user_id'] }}', '{{ $offerdetails[0]->recruiter_id }}', '{{ $offerdetails[0]->organization_id }}', '{{ $userdetails->organization_name }}')"
                class="rounded-pill ss-apply-btn py-2 border-0 px-4" style="cursor: pointer;">Chat Now</a>
        </li>
    {{-- @endif --}}
</ul>
        ...
        @foreach ($offerdetails as $value)
            ...
            >
                ...
                <ul class="ss-cng-appli-slid-ul3">
                    ...
                    <li>
                        <h6>{{ number_format($value->hours_per_week) }} hrs/wk</h6>
                    </li>
                </ul>
                <h5>{{ $value->job_id }}</h5>
            </div>
