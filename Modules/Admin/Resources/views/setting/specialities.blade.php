                    ...
                    <div class="col-sm-12">
                        <div class="card-box table-responsive">
                            <div class="form-group col-md-4 col-sm-4">
                                <select name="speciality" id="profession" class="form-control">
                                    @forelse ($professions as $m)
                                    <option value="{{$m->id}}">{{$m->title}}</option>
                                    @empty
                                    <option value="">No Profession found</option>
                                    @endforelse
                                </select>
                            </div>
                            <div class="row float-right">
                                <div class="col-md-12 col-sm-12 d-flex justify-content-start">
                                    <a class="btn btn-success" href="{{ route('add-speciality') }}">Add New</a>
...
$(document).ready(function(){
    ...
    function initializeDT(filter) {
        dynamicTable = $('#listing-table').DataTable({
            processing: true,
            serverSide: true,
            aaSorting: [[2,"ASC"]],
            bSortable: true,
    ...
    $('#profession').on('change',function() {
        ...
        if (dynamicTable) {
            // If dynamicTable instance already exists, destroy it
            dynamicTable.destroy();
        }
        initializeDT(filter);
    });
    // $('#profession').children().eq(0).select();
    $('#profession option:first').prop('selected', true).trigger('change');
});
</script>
<script>
// check all
